...
def generate_data(n=100, datatype='', seed = 0, ANR_e_scale = None):
    """
    Generate data (X,y)
    ...
    if datatype == 'orange_skin': 
        ...
        # EDIT
        if ANR_e_scale is None:
            scale = 1
        else:
            scale = ANR_e_scale
        
        y = -2 * np.sin(2*X[:,0]) + np.maximum(X[:,1], 0) + X[:,2] + np.exp(-X[:,3]) + np.random.normal(scale=scale, size=n) 
