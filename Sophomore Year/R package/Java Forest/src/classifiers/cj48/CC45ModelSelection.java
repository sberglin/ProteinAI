 ...
 */
public class CC45ModelSelection extends ModelSelection {
   ...
   */
  public CC45ModelSelection(int minNoObj, Instances allData,
  ...
  public final ClassifierSplitModel selectModel(Instances data) {
    ...
    try {
      ...
      // EDIT: Chi-squared evaluator and results
      ...
      // EDIT: Using chi-squared tests instead
      bestChi = 0;
      for (i = 0; i < data.numAttributes(); i++) {
        if ((i != (data).classIndex()) && (currentModel[i].checkModel())) {
          // Use 1E-3 here to get a closer approximation to the original
          // implementation.
          // EDIT: Checking chi squared statistics and not infoGain
          if ((currentModel[i].infoGain() >= (averageInfoGain - 1E-3))
            && Utils.gr(chi.evaluateAttribute(i), bestChi)) {
            bestModel = currentModel[i];
            bestChi = chi.evaluateAttribute(i);
          }
        }
      }

      // Check if useful split was found.
      // EDIT: checks if split is useful at significance level of 0.05
      double df = (data.numAttributes()-1)*(data.numClasses()-1);
      double pval = Statistics.chiSquaredProbability(bestChi, df);
      alpha = 0.05;
      if (Utils.gr(pval, alpha)) {
        return noSplitModel;
      }
